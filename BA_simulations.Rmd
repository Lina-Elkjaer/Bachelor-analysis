...
# Simulate a population of "good" reaction times:
  ...
  rt_dist1_base_ex <- rexgauss(50, mu=1680, sigma=100, 200, positive = F)  #NB Chose to use centiseconds instead of miliseconds because it is responsetimes, not reaction times. Means based of the run we did on ourselves.
  # keep positive values only:
  rt_dist1_base_ex <- rt_dist1_base_ex[rt_dist1_base_ex > 0]

  rt_dist0_base_ex <- rexgauss(50, mu=1020, sigma=100, 200, positive = F)
  # keep positive values only:
  ...
  data_ex <- data  %>% 
    mutate(rt_dist = ifelse(gender==0,
           ...
           RT = rnorm(800, mean=rt_dist, sd=500),
  ...
  data_ex %>%
    ...
    labs(x = "Trial number", y = "Response time")
    #scale_color_manual(name="Gender", breaks=c("0", "1"), values = c("lightcoral", "lightseagreen"))
...
# Simulation with different means for gender and individual differences by gender 
  ...
  rt_dist1_base <- rnorm(50, mean=1608, sd=200)
  # keep positive values only:
  rt_dist1_base <- rt_dist1_base[rt_dist1_base > 0]

  #for gender=0:
  rt_dist0_base <- rnorm(50, mean=1020, sd=200)
  # keep positive values only:
