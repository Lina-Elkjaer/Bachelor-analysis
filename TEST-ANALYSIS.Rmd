...
df_all_wide <- preproc_function("/Users/linaelkjaerpedersen/Desktop/COBE-Experiment-sync/data/all_apps_wide_2021-11-29.csv")
...
sound_preproc_function <- function(filename) {
  ...
  sounddf <- read_csv(file = filename)
    ...
    sounddf <- sounddf %>% 
       mutate(volume = ifelse(volume==0,startSound/10,volume),
         startSound= ifelse(startSound > 0 & startSound < 10, response, startSound),
         response = ifelse(response == "Q" | response == "none"| response ==  "Keypad9", response, RT),
         RT = ifelse(...11=="NA", RT, ...11),
         ...11 = NULL) %>% 
    group_by(stimulus)%>% 
    mutate(
      RT = as.numeric(RT),
      RT_new = sum(RT),
      startms = as.vector(startSound),
      ) %>% 
    filter(RT_new==RT) %>% 
    filter(!(RT==0 & soundLevel!=4)) %>% 
